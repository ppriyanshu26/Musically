...
const playMusic = (track, pause = false) => {
    currentsong.src = `/songs/${currFolder}/` + track; // Update URL construction
    if (!pause) {
        currentsong.play();
        play.src = "pause.svg";
    } else {
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (e.href.includes("/songs")) {
            ...
            cardcontainer.innerHTML += `<div data-folder="${folder}" class="card">
                <div class="play">
                    <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="#000">
...
async function main() {
    ...
    currentsong.addEventListener("timeupdate", () => {
        ...
        // Calculate the percentage of playback completed
        const percentagePlayed = (currentsong.currentTime / currentsong.duration) * 100;
    
        // Update the position of the seek bar
        document.querySelector(".circle").style.left = percentagePlayed + "%";
    });
    ...
    currentsong.addEventListener("ended", () => {
        // Check if loop mode is disabled
        if (!currentsong.loop) {
    ...
    document.querySelector(".loop").addEventListener("click", (e) => {
        const loopButton = e.target;
        const isLooped = loopButton.src.includes("looped.svg");
    
        // Toggle between loop and looped icons
        if (isLooped) {
            loopButton.src = loopButton.src.replace("looped.svg", "loop.svg");
        } else {
            loopButton.src = loopButton.src.replace("loop.svg", "looped.svg");
        }
    
        // Toggle looping of the current song
        currentsong.loop = !isLooped;
    });
...
function shufflePlaylist() {
    for (let i = songs.length - 1; i > 0; i--) {
        ...
        [songs[i], songs[j]] = [songs[j], songs[i]]; // Swap songs[i] and songs[j]
    }
}

// Add event listener for shuffle button
document.querySelector(".shuffle").addEventListener("click", (e) => {
    const shuffleButton = document.querySelector(".shuffle img"); // Get the img element inside the shuffle button
    const isShuffled = shuffleButton.src.includes("shuffled.svg");

    // Toggle between shuffle and shuffled icons
    if (isShuffled) {
        shuffleButton.src = shuffleButton.src.replace("shuffled.svg", "shuffle.svg");
    } else {
        shuffleButton.src = shuffleButton.src.replace("shuffle.svg", "shuffled.svg");
    }
...
// Add event listener for loop button
